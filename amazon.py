...
def get_prod_objects(listings):
    ...
    for product in tqdm(listings):
        ...
        if name != None and link != None and price != None:
            ...
            try:
                ...
                try:
                    ...
                    soup_desc = BeautifulSoup(product_desc.content, "lxml", parse_only=strainer)
                    description = soup_desc.findAll("table", attrs={
                        "class": "a-bordered a-horizontal-stripes aplus-tech-spec-table"})
                    content = ""
        ...
        storage_object = StorageBoi(pricE=price, urL=link, descriptioN=content, titlE=name, categorY=category,
                                    datE=None)
